...
Status CompactionJob::Run() {
  ...
  constexpr double kMB = 1048576.0;
		...
		fprintf(stdout, "%lu %lu-Level CompactionJob::Run() [ %lu %lu %lu %lu %lu %lu %lu ] to level-%d total output files %lu (write %1.f MB))\n", 
				db_options_.clock->NowMicros(),	 		
				compact_->compaction->num_input_levels(),	
				compact_->compaction->num_input_files(0),
				compact_->compaction->num_input_files(1),
				compact_->compaction->num_input_files(2),
				compact_->compaction->num_input_files(3),
				compact_->compaction->num_input_files(4),
				compact_->compaction->num_input_files(5),
				compact_->compaction->num_input_files(6),
				compact_->compaction->output_level(),
				compact_->num_output_files,
				compact_->total_bytes/kMB);		
