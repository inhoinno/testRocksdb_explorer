...
testname = "quicktest_random_read"
''' 
                    ...
                    for l in l0_vars:
                        for minmerge in min_merge_width_vars:
                            for b in benchmark:
                                ...
                                os.system("echo =========== {0} Compaction Style = {1} key size = {2}, value size = {3}, block cache size = {4} l0trigger {7} min_merge width {8} =========== >>{5}/{6}".format("fillrandom",i,k,v,c,RESULT_PATH,RESULT_FILE,l,minmerge))
                                ...
                                os.system("cp ../mnt/LOG ../logs/LOG_{0}_COMPACTION_{1}_k{2}_v{3}_c{4}_l{5}_m{6}_count{7}".replace("COMPACTION","LVL" if i==0 else "UNIV").format(testname,"fillrandom",k,v,c,l,minmerge,count))
                                ...
                                for rdonly in rdonly_vars:
                                    ...
                                    if (b=="readrandom") | (b=="readseq") | (b=="seekrandom"):
                                        bench = "../db_bench --db=$DEV_PATH --use_direct_io_for_flush_and_compaction=true --benchmarks=$BENCHMARK --key_size=$k --value_size=$v --num=$NUM --compaction_style=$COMPACTIONSTYLE --use_existing_db=true --readonly=$RDONLY --histogram --statistics --cache_size=$c --level0_file_num_compaction_trigger=$l --universal_min_merge_width=$min_merge --report_interval_seconds=1 --report_file=$RESULT_PATH/$NAME >> $RESULT_PATH/$RESULT_FILE"
                                        bench=bench.replace("$RDONLY", "false" if rdonly == 0  else "true")
                                    #IF END
