...
PARSER_RESULT4="test7_WA_COMPACTIONSTYLE_BENCH_kKEY_vVALUE.csv"
    ...
    for k in key_vars:
        for v in value_vars:
            for b in benchmark:
                ...
                wa_parser=wa_parser.replace("$PARSER", PARSER,2).replace("$RESULT_PATH",RESULT_PATH,2).replace("$RESULT_FILE",RESULT_FILE,2).replace("$PARSER_RESULT",PARSER_RESULT4)
