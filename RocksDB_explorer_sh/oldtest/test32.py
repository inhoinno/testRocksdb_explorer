...
NUM=5000000
RESULT_FILE="test8_COMPACTIONSTYLE_BENCH_kKEY_vVALUE.txt"
NAME="test8_report_COMPACTIONSTYLE_BENCH_kKEY_vVALUE.csv"
        ...
        for v in value_vars:
            for b in benchmark:
                ...
                if b == "readrandom":
                    ...
                    bench = "../db_bench --db=$DEV_PATH --use_direct_io_for_flush_and_compaction=true --benchmarks=$BENCHMARK --key_size=$k --value_size=$v --num=$NUM --compaction_style=$COMPACTIONSTYLE --use_existing_db=true --readonly=true --write_buffer_size=$MEM_SIZE --max_write_buffer_number=4 --target_file_size_base=$MEM_SIZE --histogram --statistics  --report_interval_seconds=1 --report_file=$RESULT_PATH/$NAME >> $RESULT_PATH/$RESULT_FILE"
                elif b == "fillrandom": 
                    #RESULT_FILE = RESULT_FILE.replace("BENCH","fillrandom")
                    #NAME = NAME.replace("BENCH","fillrandom")
                    bench = "../db_bench --db=$DEV_PATH --use_direct_io_for_flush_and_compaction=true --benchmarks=$BENCHMARK --key_size=$k --value_size=$v --num=$NUM --compaction_style=$COMPACTIONSTYLE --write_buffer_size=$MEM_SIZE --max_write_buffer_number=4 --target_file_size_base=$MEM_SIZE --histogram --statistics  --report_interval_seconds=1 --report_file=$RESULT_PATH/$NAME >> $RESULT_PATH/$RESULT_FILE"
                PARSER_RESULT2="test7_99_COMPACTIONSTYLE_BENCH_kKEY_vVALUE.csv"

                bench = bench.replace("$DEV_PATH",DEV_PATH).replace("$BENCHMARK",BENCHMARK).replace("$k",str(k),2).replace("$v",str(v),2).replace("$NUM",str(NUM)).replace("$COMPACTIONSTYLE",str(i)).replace("$NAME",NAME).replace("$RESULT_PATH", RESULT_PATH,2).replace("$RESULT_FILE",RESULT_FILE).replace("$MEM_SIZE", str(MEM_SIZE) ,2)
                #print(bench)
...
PARSER_RESULT="test8_COMPACTIONSTYLE_BENCH_kKEY_vVALUE.csv"
PARSER_RESULT2="test8_99_COMPACTIONSTYLE_BENCH_kKEY_vVALUE.csv"
PARSER_RESULT3="test8_avg_COMPACTIONSTYLE_BENCH_kKEY_vVALUE.csv"
