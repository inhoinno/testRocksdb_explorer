...
echo ========== Operation Compaction on for Latency =========== >> $RESULT_PATH/compaction_off_latency_result.txt
../db_bench --db=$DEV_PATH --use_direct_io_for_flush_and_compaction=true --benchmarks=$BENCHMARK --histogram --value_size=$VALUE_SIZE --num=$NUM --disable_auto_compactions=true --level0_file_num_compaction_trigger=-1 --statistics>> $RESULT_PATH/compaction_off_latency_result.txt
