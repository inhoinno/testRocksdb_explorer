...
RESULT_FILE="test12_readamp_COMPACTIONSTYLE_BENCH_kKEY_vVALUE.txt"
NAME="test12_report_readamp_COMPACTIONSTYLE_BENCH_kKEY_vVALUE.csv"
...
compactionstyle=[0,1]
benchmark=["fillseq","readrandom","readseq"]
I=0
bench=""
    ...
    for k in key_vars:
        for v in value_vars:
            for b in benchmark:
                ...
                bench = bench.replace("$DEV_PATH",DEV_PATH).replace("$BENCHMARK",BENCHMARK).replace("$k",str(k),2).replace("$v",str(v),2).replace("$NUM",str(d[k][v])).replace("$COMPACTIONSTYLE",str(i)).replace("$NAME",NAME).replace("$RESULT_PATH", RESULT_PATH,2).replace("$RESULT_FILE",RESULT_FILE)
                #print(bench)
                ...
                RESULT_FILE="test12_readamp_COMPACTIONSTYLE_BENCH_kKEY_vVALUE.txt"
                NAME="test12_readamp_report_COMPACTIONSTYLE_BENCH_kKEY_vVALUE.csv"
...
PARSER_RESULT="test12_COMPACTIONSTYLE_BENCH_kKEY_vVALUE.csv"
PARSER_RESULT2="test12_99_COMPACTIONSTYLE_BENCH_kKEY_vVALUE.csv"
PARSER_RESULT3="test12_avg_COMPACTIONSTYLE_BENCH_kKEY_vVALUE.csv"
