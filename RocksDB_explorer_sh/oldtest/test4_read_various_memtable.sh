...
RESULT_FILE="test4_64various_memtable.txt"

print > $RESULT_PATH/$RESULT_FILE
echo ========== fillrandom 64MB] Test4 MemTable size Compaction Latency =========== >> $RESULT_PATH/$RESULT_FILE
...
print >> $RESULT_PATH/$RESULT_FILE
echo ========== readrandom 64MB] Test4 MemTable size Compaction Latency =========== >> $RESULT_PATH/$RESULT_FILE
../db_bench --db=$DEV_PATH --use_direct_io_for_flush_and_compaction=true --benchmarks=$BENCHMARK --histogram --value_size=$VALUE_SIZE --num=$NUM --disable_auto_compactions=false --level0_file_num_compaction_trigger=$L0_FILE_NUM --write_buffer_size=$MEM_SIZE --target_file_size_base=$MEM_SIZE --use_existing_db >> $RESULT_PATH/$RESULT_FILE

let MEM_SIZE=32*1024*1024
MEM_NUM=4
RESULT_FILE="test4_32various_memtable.txt"

BENCHMARK=fillrandom,levelstats

print > $RESULT_PATH/$RESULT_FILE
echo ========== fill readrandom 32MB] Test4 MemTable size Compaction Latency =========== >> $RESULT_PATH/$RESULT_FILE
...
#MEM_NUM=8
#RESULT_FILE="test4_16various_memtable.txt"

#print > $RESULT_PATH/$RESULT_FILE
#echo ========== readrandom 16MB] Test4 MemTable size Compaction Latency =========== >> $RESULT_PATH/$RESULT_FILE
#../db_bench --db=$DEV_PATH --use_direct_io_for_flush_and_compaction=true --benchmarks=$BENCHMARK --histogram --value_size=$VALUE_SIZE --num=$NUM --disable_auto_compactions=false --level0_file_num_compaction_trigger=$L0_FILE_NUM --write_buffer_size=$MEM_SIZE --max_write_buffer_number=$MEM_NUM --target_file_size_base=$MEM_SIZE >> $RESULT_PATH/$RESULT_FILE
