...
LOG_FILE = "LOG_{0}_COMPACTIONSTYLE_BENCH_kKEY_vVALUE_cBLOCKCACHE".format(testname)
...
compactionstyle=[0,1]
benchmark=["fillrandom","fillseq","readrandom","readseq","seekrandom"]
#benchmark=["fillseq", "readseq"]
I=0
bench=""
tailname = "COMPACTIONSTYLE_BENCH_kKEY_vVALUE_cBLOCKCACHE.csv"
        ...
        for v in value_vars:
            for c in cache_vars:
                for b in benchmark:
                    ...
                    LOG_FILE = LOG_FILE.replace("KEY",str(k)).replace("VALUE",str(v)).replace("BLOCKCACHE",str(c)).replace("BENCH",b)
