...
testname = "uniformtest_l0triggernum_SA_t_fixed"
''' 
we first calculate Space Amplification by adjusting l0_file_num_compaction_trigger 2 to 8 in step=2
however, l0_file_num_compaction_trigger is a option not only a threshold to trigger l0 compaction 
but threshold for # of sorted run of entire DB.

for time saving, we have some change from default 
    
    
                                    <Bench file>    <Parser file>
    1. 10GB * 5                    (Done)    |      (Done)
    2. Memtable 32MB               (Done)    |      (NoNeed) 
    3. SST 32MB                    (Done)    |      (NoNeed)
    4. MAX LEVEL = 5               (Done)    |      (NoNeed)


We expect normalized Space amplification result chaning the min_merge_width and Level0 trigger num
...
RESULT_FILE="{0}_COMPACTIONSTYLE_BENCH_kKEY_vVALUE_cBLOCKCACHE_lLEVEL0_mMINMERGE_countCOUNT.txt".format(testname)
OLDRESULT_FILE = RESULT_FILE

NAME="{0}_report_COMPACTIONSTYLE_BENCH_kKEY_vVALUE_cBLOCKCACHE_lLEVEL0_mMINMERGE_countCOUNT.csv".format(testname)
OLDNAME=NAME
...
compactionstyle=[0,1]
benchmark=["fillrandom","readrandom","seekrandom"]
#benchmark=["fillseq"]
I=0
bench=""
                        ...
                        for l in l0_vars:
                            for minmerge in min_merge_width_vars:
                                ...
                                if (b == "readrandom") | (b=="readseq") | (b=="seekrandom"):
                                    bench = "../db_bench --db=$DEV_PATH --use_direct_io_for_flush_and_compaction=true --benchmarks=$BENCHMARK --key_size=$k --value_size=$v --num=$NUM --compaction_style=$COMPACTIONSTYLE --use_existing_db=true --prefix_size=14 --keys_per_prefix=8 --readonly=true --level0_file_num_compaction_trigger=$l --universal_min_merge_width=$min_merge --write_buffer_size=$MEM_SIZE --target_file_size_base=$MEM_SIZE --num_levels=5 --histogram --statistics --cache_size=$c --read_amp_bytes_per_bit=16 --report_interval_seconds=1 --report_file=$RESULT_PATH/$NAME >> $RESULT_PATH/$RESULT_FILE"
                                elif (b == "fillrandom") | (b=="fillseq"): 
                                    bench = "../db_bench --db=$DEV_PATH --use_direct_io_for_flush_and_compaction=true --benchmarks=$BENCHMARK --key_size=$k --value_size=$v --num=$NUM --compaction_style=$COMPACTIONSTYLE --histogram --statistics --cache_size=$c --level0_file_num_compaction_trigger=$l --universal_min_merge_width=$min_merge --write_buffer_size=$MEM_SIZE --target_file_size_base=$MEM_SIZE --num_levels=5 --prefix_size=14 --keys_per_prefix=8 --read_amp_bytes_per_bit=16  --report_interval_seconds=1 --report_file=$RESULT_PATH/$NAME >> $RESULT_PATH/$RESULT_FILE"

                                bench = bench.replace("$DEV_PATH",DEV_PATH).replace("$BENCHMARK",BENCHMARK).replace("$k",str(k),2).replace("$v",str(v),2).replace("$NUM",str(d[k][v])).replace("$COMPACTIONSTYLE",str(i)).replace("$NAME",NAME).replace("$RESULT_PATH", RESULT_PATH,2).replace("$RESULT_FILE",RESULT_FILE).replace("$c",str(C)).replace("$l",str(l)).replace("$min_merge",str(minmerge)).replace("$MEM_SIZE", str(MEM_SIZE),2)
                                #print(bench)
                                os.system("print > {0}/{1}".format(RESULT_PATH,RESULT_FILE))
                                os.system("echo =========== {0} Compaction Style = {1} key size = {2}, value size = {3}, block cache size = {4} l0trigger {7} min_merge width {8}=========== >>{5}/{6}".format(b,i,k,v,c,RESULT_PATH,RESULT_FILE,l,minmerge))
                                ...
                                os.system("cp ../mnt/LOG ../logs/LOG_{0}_COMPACTION_{1}_k{2}_v{3}_c{4}_l{5}_m{6}_count{7}".replace("COMPACTION","LVL" if i==0 else "UNIV").format(testname,b,k,v,c,l,minmerge,count))
...
print("*********** bench FIN ****************")
os.system("python3 parser_integrated_L0_MINMERGE.py {0}".format(testname))
'''
